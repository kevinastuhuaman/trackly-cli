...
4. Use `trackly_get_job` for the exact posting record and `trackly_get_job_brief` for a decision-ready summary.
...
6. Call `trackly_update_status` only after the user chooses a job or explicitly asks to update its status.
