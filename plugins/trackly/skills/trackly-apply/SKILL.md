...
6. If `memberIds` is empty, never send a snapshot. For `nextAction: use_active_target`, call `trackly_start_or_resume_apply` again with the returned `activeTarget`, the same browser surface, and a fresh idempotency key. For `nextAction: advance_or_refresh`, call `trackly_get_apply_work` without `snapshot` and obey its authoritative next action or terminal state. Stop on any other empty-binding result rather than inventing work.
