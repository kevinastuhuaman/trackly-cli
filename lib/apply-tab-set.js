...
const APPLY_TAB_SET_FAILURE_CODES = Object.freeze({
  ...
  AMBIGUOUS_CROSS_TYPE_TAB_ID: 'ambiguous_cross_type_tab_id',
...
function hasCrossTypeCollision(collections) {
  const typesByCanonicalId = new Map();
  for (const tabId of collections.flat()) {
    const canonical = canonicalizeTabId(tabId);
    const types = typesByCanonicalId.get(canonical) ?? new Set();
    types.add(typeof tabId);
    typesByCanonicalId.set(canonical, types);
  }
  return [...typesByCanonicalId.values()].some((types) => types.size > 1);
}
...
function validateApplyTabKeepSet({
  ...
  if (hasCrossTypeCollision([
    expectedTabIds, keepTabIds, controllerInventory.tabIds, userInventory.tabIds,
  ])) addFailure(APPLY_TAB_SET_FAILURE_CODES.AMBIGUOUS_CROSS_TYPE_TAB_ID);
