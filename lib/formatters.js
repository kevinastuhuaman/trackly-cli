...
function resolveJobDateBundle(job = {}) {
  ...
  const presentation = projectedKind && projectedDay
    ? {
      label: { posted: 'Posted', reposted: 'Reposted', found: 'Found' }[projectedKind],
      value: projectedDay,
    }
    : legacyPostedDay
      ? { label: 'Posted', value: legacyPostedDay }
      : { label: 'Found', value: foundDay || 'unknown' };
  return {
    projectedKind,
    projectedDay,
    originalDay,
...
function jobDatePresentation(job = {}) {
  return resolveJobDateBundle(job).presentation;
}
...
function jobDateDetailLines(job = {}) {
  ...
  else if ((!projectedKind || !projectedDay) && legacyPostedDay) {
    lines.push(`Posted: ${legacyPostedDay}`);
  }
