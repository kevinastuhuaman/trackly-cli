...
function evaluateApplyCompatibility(protocol, clients) {
  ...
  const minimumCliVersion = typeof protocol?.compatibleCliMinimumVersion === 'string'
    ...
    : null;
  const accessibleExecutionEnabled = protocol?.batchOrchestration?.accessibleExecution?.enabled === true;
  const mcpContractCompatible = protocol?.mcpContractVersion === MCP_CONTRACT_VERSION
    || (!accessibleExecutionEnabled && protocol?.mcpContractVersion === '3.4.0');
