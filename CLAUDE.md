...
mcp/server.js        # MCP server: 54 tools, launched via `trackly mcp`
...
3. `mcp/server.js` creates an `McpServer` with 54 tools, connects via `StdioServerTransport`
...
The 54 MCP tools include the complete search, network, profile, and Trackly Apply set documented in `docs/trackly-tools.md`. Keep this count synchronized with `mcp/server.js`, `README.md`, and the docs-drift tests.
