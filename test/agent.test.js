...
test('agent setup installs one canonical skill and links both clients', () => {
  withTempAgentHome(() => {
    ...
    assert.equal(result.skillVersion, '4.5.0');
...
test('agent doctor inspection fails closed on modified, missing, or extra managed skill content', () => {
  for (const mutation of ['modified', 'missing', 'extra']) {
    withTempAgentHome(() => {
      ...
      assert.equal(inspection.installedSkillVersion, '4.5.0', mutation);
...
test('Apply execution mode makes managed skill 4.0.0 stale and setup installs 4.5.0', () => {
  withTempAgentHome(() => {
    ...
    const setup = agent.setupAgent('codex');
    assert.equal(setup.skillVersion, '4.5.0');
    const after = agent.inspectClient('codex');
    assert.equal(after.installed, true);
    assert.equal(after.installedSkillVersion, '4.5.0');
...
test('agent doctor compatibility enforces the protocol minimum installed skill version', () => {
  const installed = [{
    client: 'codex',
    installed: true,
    installedSkillVersion: '4.5.0',
  }];
  const current = agent.evaluateApplyCompatibility({
    version: '3.6.0',
    mcpContractVersion: '3.7.1',
  ...
  const future = agent.evaluateApplyCompatibility({
    ...
    compatibleCliMinimumVersion: '0.8.2',
    compatibleSkillMajor: 4,
    compatibleSkillMinimumVersion: '4.6.0',
  }, installed);
...
test('agent doctor accepts the local MCP contract during an explicit overlap window', () => {
  ...
  const result = agent.evaluateApplyCompatibility({
    ...
    preferredMcpContractVersion: '3.7.1',
    compatibleMcpContractVersions: ['3.7.1'],
  ...
  assert.equal(result.preferredMcpContractVersion, '3.7.1');
  assert.deepEqual(result.compatibleMcpContractVersions, ['3.7.1']);
});
