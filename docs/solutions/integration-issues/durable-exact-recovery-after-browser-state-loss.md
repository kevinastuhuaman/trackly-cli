...
Submit remains a hard human boundary. The skill forbids the agent from clicking Submit even when the user previously approved submission (`skills/trackly-apply/SKILL.md`, “Non-negotiable rules”). The agent verifies the current form and leaves the review tab visible. After any grouped human submission statement, first call `trackly_list_apply_review_handoffs`, resolve the statement to the named receipt or sole active receipt, classify every member in that exact receipt, and call `trackly_claim_apply_review_handoff`; if more than one receipt could match, ask instead of guessing. Only then record submission evidence or outcomes for members with a real success state or explicit member-bound user confirmation (`skills/trackly-apply/SKILL.md`, “Review handoff”; `skills/trackly-apply/references/review-handoff.md`, “Group submission reconciliation”).
