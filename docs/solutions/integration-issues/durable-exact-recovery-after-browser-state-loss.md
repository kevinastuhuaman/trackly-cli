...
At browser-turn end, execute—not merely advertise—the preservation path proven before mutation. Either call the one session finalizer with the complete controller-owned plus user-owned live-tab keep set, or perform and verify the documented durable handoff for every live application tab. Require the finalizer result or every per-tab persistence receipt before reporting the recovered review handoff complete (`skills/trackly-apply/SKILL.md`, “Non-negotiable rules”; `skills/trackly-apply/references/browser-lifecycle.md`, “Preserving browser state at turn end”).
