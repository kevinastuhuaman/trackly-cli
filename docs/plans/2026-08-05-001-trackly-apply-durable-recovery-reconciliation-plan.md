...
- AE4. **Mixed exact recovery set**
  ...
  - **Then:** Identity assertion covers all three, per-member results explain each disposition, and the recovery execution contains only the recoverable member with no replacement.
...
- KTD3. **Model exact recovery as a new execution mode over stable source candidates.** Use `recover_exact_members` with `sourceExecutionId`, exact execution-candidate IDs proven inside the source snapshot hash, optimistic revision, and idempotency key. Persist `source_execution_id`, `source_candidate_id`, and the attempt-specific `source_member_id` when present. The identity assertion and eligibility check are atomic and all-or-nothing: if any confirmed candidate is no longer eligible, the server creates no recovery execution and no substitutes. Governs R4-R7.
