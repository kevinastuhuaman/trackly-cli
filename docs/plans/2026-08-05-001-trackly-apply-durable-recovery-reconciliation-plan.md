...
- R5. Exact recovery must assert the requested source membership, report each member's eligibility, create no substitutes, reuse valid run identity, and require a fresh browser binding and inspection epoch. The shipped contract applies these checks atomically to the whole confirmed set as superseded by the KTD3 addendum below; a partially eligible set creates no recovery execution.
