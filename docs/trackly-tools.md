...
- **get_more_tools** — Report what the agent wanted to do when no Trackly tool fit, so Trackly can improve its MCP surface. Available on the analytics-enabled local MCP server.
