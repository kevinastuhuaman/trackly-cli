...
- **trackly_record_apply_execution_dispositions** — Record typed, value-free live-probe classifications for the current wave. Every item requires the exact `batchId`, `memberId`, `runId`, and `browserSurface`; cache/static scheduling records are server-owned and cannot be submitted through MCP.
