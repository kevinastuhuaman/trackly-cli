...
function registerApplyTools(
  ...
  server.registerPrompt('trackly-apply', {
    ...
    messages: [{
      ...
      content: {
        type: 'text',
        text: 'Protocol capability clarification: stale-projection reconciliation for success-page or explicit user-confirmation evidence is available only when both the fetched Apply protocol and the stored run.protocolVersion are 3.3.2 or newer. Preserve an existing success_page confirmation when a later user_confirmation triggers repair. If either version is 3.3.1, preserve the success tab and use only the normal review-ready transition when valid; otherwise report that the backend must update. Read and write prior-employer answers through the canonical global keys employment.most_recent_company and employment.most_recent_title only when the fetched profile schema exposes those exact keys. If an exposed key is unknown, ask once and sync only the confirmed value. If a key is absent, do not PATCH it; retain the answer only for the current form and report the schema gap.',
      },
    }, {
      role: 'user',
      content: {
