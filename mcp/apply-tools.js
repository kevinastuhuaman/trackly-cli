...
function registerApplyTools(
  ...
  server.registerPrompt('trackly-apply', {
    ...
    messages: [{
      ...
      content: {
        type: 'text',
        text: 'External inbox receipt preflight: Require skill 4.2.7 or newer. Trackly remains mailbox-blind: Trackly never receives mailbox access, credentials, connection state, raw messages, message metadata, receipt identifiers, or URLs. Before mutating the first form in a newly frozen batch, make one non-blocking offer to check for prior-application receipts using a separately connected agent-side inbox tool. Proceed only after explicit batch-scoped consent; connector availability is not consent and consent is never saved to the Trackly profile. If the user declines, does not opt in, or no connector is callable, skip the check and continue without blocking browser work. Scope search and completion only to executable frozen members without static exclusions; retained inactive, insecure-URL, or protocol-declared manual-only members are skipped and never require a forbidden run. Keep only value-free preflight state in the private local batch ledger. On recovery of consented_pending, verify the same batch and require the user to re-select or confirm the exact inbox connector and account; never substitute a client default. Mark completed only after no positive match exists or every executable positive match is durably recorded against the exact member and run and, when submission authority exists, reconciled. Otherwise retain consented_pending and preserve any matched member without form mutation while unaffected siblings continue. With consent, use the smallest bounded query for exact requisition identity plus the same employer or verified ATS tenant/sender identity, or employer plus exact or near-exact role and an approved pre-batch lookback that can contain prior submissions. A bare requisition ID is never sufficient. Use the known posting-to-freeze interval for each job; when no trustworthy posting timestamp exists, require a historical range the user selects and never search the whole mailbox. Keep raw results local. An exact requisition plus matching employer or verified ATS identity may follow the normal verified-receipt path. Without a requisition ID, a weaker employer, role, and approved-lookback match is not actionable and must not be recorded as provider_receipt_detected until the user explicitly confirms that it belongs to the current batch member. Same-company/different-role evidence is negative for the current member. A receipt proves identity only and never replaces a visible success page or explicit user confirmation as submission authority. Record only the locally hashed provider_receipt_detected proof through the existing redacted evidence tool after the exact run and browser binding exist.',
      },
    }, {
      role: 'user',
      content: {
